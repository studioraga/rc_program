...
## Stage 28 — Compare Alternative Implementations

**Status: COMPLETE AND VALIDATED**

Stage 28 is intentionally implemented in a separate source file:

```text
ranked_cached_cmp_alternative.c
```

`ranked_cache.c` remains unchanged from Stage 27.

The purpose of Stage 28 is to compare five complete implementations under the same deterministic cache semantics rather than continuing to mutate the main incremental implementation.

### Implemented alternatives

| Version | Architecture | Lookup | Rank update | Minimum | Eviction |
|---|---|---:|---:|---:|---:|
| A | linear array baseline | O(N) | O(1)* | O(N) | O(N) |
| B | hash + linear minimum | expected O(1) | O(1)* | O(N) | O(N) |
| C | hash + AVL tree | expected O(1) | O(log N) | O(log N) | O(log N) |
| D | hash + lazy heap | expected O(1) | O(log M) append | amortized stale cleanup | amortized + O(log M) |
| E | hash + indexed heap | expected O(1) | O(log N) | O(1) | O(log N) |

`*` Rank mutation is O(1) once the resident has already been located.

`M` is the lazy-heap record count and may grow far beyond resident count `N`.

### Version A — linear baseline

Version A contains only a compact resident array.

```text
lookup          linear scan
minimum         linear scan
rank mutation   direct field assignment
```

It is the simplest correctness baseline and retains excellent locality for small caches, but both lookup and eviction scale linearly.

### Version B — hash + linear minimum

Version B adds an open-addressing hash table pointing to stable resident slots.

This removes the key-lookup bottleneck:

```text
lookup expected O(1)
```

but deliberately leaves minimum selection as a full active-slot scan:

```text
eviction O(N)
```

It demonstrates that optimizing lookup alone does not solve the eviction bottleneck.

### Version C — hash + AVL tree

Version C combines expected-O(1) hash lookup with an AVL tree ordered by:

```text
(rank,key)
```

A rank change removes the old ordered-tree key and inserts the new one, giving O(log N) repair.

The current implementation obtains minimum by walking to the leftmost AVL node, therefore minimum and eviction are O(log N). A separately maintained minimum pointer could make the minimum peek O(1), but is intentionally not added here.

### Version D — hash + lazy heap

Version D performs no arbitrary in-place heap priority update.

Instead every rank change:

```text
updates the resident
increments a generation counter
pushes a new versioned heap record
leaves the old heap record stale
```

Before using the heap root, stale versions are popped until a current record appears.

This greatly simplifies update bookkeeping, but heap memory depends on update history rather than resident count.

A focused validation demonstrates:

```text
4 residents
100 rank updates to one resident

Version D heap records = 104
Version E heap records = 4
```

### Version E — hash + indexed heap

Version E is the final optimized architecture developed incrementally in the main project.

Each stable resident stores:

```text
heap_index
```

which provides direct O(1) resident-to-heap-position mapping.
...
while keeping exactly one heap record per resident.

### Cross-version deterministic equivalence

All five versions use the same:

- deterministic database,
- hit-only dynamic rank update rule,
- lower/unchanged/higher rank scenarios,
- `(rank,key)` minimum ordering,
- miss/eviction/insertion semantics.

The main Stage 28 equivalence workload uses:
...
All Versions A–E must match those statistics, the returned-entry checksum, and the final logical resident state.

Version D ends this churn workload with:

```text
resident entries = 8
lazy heap records = 774
stale discards    = 171
```

while Version E ends with:

```text
resident entries = 8
indexed heap records = 8
```

This makes the lazy-heap versus indexed-heap memory trade-off directly observable.
...
Optimized semantic-equivalence build:
...
Detailed data-structure diagrams, complexity explanations, and trade-off analysis are in:
...
Stage 28 deliberately does not replace the main `ranked_cache.c` implementation and does not introduce another optimization stage.
